...
impl DataSourceHandler for WinitState {
    ...
    fn send_request(
        ...
        let Some(data) = self.data_transfer_state.send_data(source) else {
...
/// A [`DataTransfer`] describing the current clipboard offer.
#[derive(Debug)]
...
/// The source of the current clipboard contents, if set by this application.
#[derive(Debug)]
pub struct ClipboardSource {
    /// The `WlDataSource` serving the clipboard contents.
    data_source: CopyPasteSource,
    /// The supplied [`DataTransferSend`].
    data: Box<dyn DataTransferSend>,
}

/// The state of in-progress data transfer operations (drag-and-drop and clipboard).
...
impl DataTransferState {
    ...
    pub(crate) fn send_data(&mut self, source: &WlDataSource) -> Option<&mut dyn DataTransferSend> {
        if let Some(drag) =
